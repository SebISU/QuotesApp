...
def register():
    ...
    if form.submit.data and form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit() # can i do this in one commit?
    ...
    return render_template('users/register.html', title='Sign Up', form=form, schform=schform)
...
def login():
    ...
    return render_template('users/login.html', title='Sign In', form=form, schform=schform)
...
def user_profile(username):
    ...
    best_posts = get_best_posts_user(user, BEST_TREND)
    recent_stars = get_recent_stars_user(user, BEST_TREND)
    background_image = url_for('static',
        filename='background_pics/' + about_user.background_pic)
    return render_template('users/user_profile.html', title=username, posts=posts,
...
def update_profile(username):
    ...
    if schform.schsubmit.data and schform.validate_on_submit():
        if schform.query.data != '':
            ...
            return redirect(url_for('users.update_profile', username=username))
    form = UpdateProfileForm()
    about_user = MoreInfoUser.query.filter_by(info_author=user).first()
    if form.submit.data and form.validate_on_submit():
        # when error occurs during saving/removing pics,
        # filenames in db records can be wrong
    ...
    background_image = url_for('static',
        filename='background_pics/' + about_user.background_pic)
    return render_template('users/update_profile.html', user=user, title='Update Profile',
...
def reset_request():
    ...
    if form.submit.data and form.validate_on_submit():
        ...
        return redirect(url_for('users.login'))
    return render_template('users/reset_request.html', title='Reset Password Reqeust', form=form,
        schform=schform)
